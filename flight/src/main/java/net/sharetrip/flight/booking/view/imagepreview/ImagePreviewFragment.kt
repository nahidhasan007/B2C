...
class ImagePreviewFragment : BaseFragment<FragmentImagePreviewInFlightBinding>() {
    ...
    override fun initOnCreateView() {
        ...
        val isConfirmation = requireArguments().getBundle(ARG_IMAGE_DATA)?.getBoolean(ARG_FOR_CONFIRMATION)
    ...
    override fun onStart() {
        super.onStart()
        (activity as FlightBookingActivity).findViewById<TextView>(R.id.text_view_trip_coin).visibility =
            GONE
    }
    ...
    private fun setTitleAndSubtitle(title: String, subTitle: String) {
        (activity as FlightBookingActivity).supportActionBar?.title = title
        (activity as FlightBookingActivity).supportActionBar?.subtitle = subTitle
    }
