...
class RangeDateCalendarFragment : BaseFragment<FragmentRangeCalendarBinding>() {
    ...
    override fun initOnCreateView() {
        ...
        today = when (serviceType) {
            ServiceType.FLIGHT.serviceName -> LocalDate.now().plusDays(
        ...
        when (serviceType) {
            ServiceType.FLIGHT.serviceName ->
                flightAdvanceSearchData?.let { onGetCalendarPrice(it) }

            ServiceType.VISA.serviceName ->
                initializeUIForVisa()

            else -> initializeUIForOtherService()
        }
    }

    private fun onGetCalendarPrice(advanceSearchResponse: AdvanceSearchResponse) {
        ...
        bindView.exFourCalendar.monthHeaderBinder =
            object : MonthHeaderFooterBinder<MonthViewContainer> {
                ...
                override fun bind(container: MonthViewContainer, month: CalendarMonth) {
                    val monthTitle =
                        "${month.yearMonth.month.name.lowercase().capitalize()} ${month.year}"
